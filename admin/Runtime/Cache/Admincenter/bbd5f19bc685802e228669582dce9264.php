	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
			<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
				...
				<td><?php echo ($vo["beishu"]); ?></td>
				<?php if($vo['isdraw'] == '0'): ?><td><a style="text-decoration:none" class="ml-5" layer-url="/Game.touzhudanwei.id.<?php echo ($vo["id"]); ?>.do" title="修改单位"><?php echo ($vo["danwei"]); ?></a></td>
				<?php elseif($vo['isdraw'] != '0'): ?>
				<td><a id="is_draw"><?php echo ($vo["danwei"]); ?></a></td><?php endif; ?>
				
				<td><a style="text-decoration:none" class="ml-5" layer-url="/Game.touzhuedit.id.<?php echo ($vo["id"]); ?>.do" title="修改投注号码"><?php echo ($vo["tzcode"]); ?></a></td>
...
<div id="modalwfts" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    ...
    <div class="modal-body">
        <p>
        <textarea id="_wfts_remark" class="textarea radius" placeholder="投注内容...">123123</textarea>
        </p>
    </div>
...
$("#is_draw").click(function(){
  alert("吊毛，已经开奖了，不能修改啦");
});
