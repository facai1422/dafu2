	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
            ...
            <tr class="text-c">
                ...
                <td>
                    {if condition="$vo.sys eq 1"}
                    <span style="color:blue">后台系统管理，</span>
                    {/if}
                    {if condition="$vo.bank eq 1"}
                    <span style="color:#8bc34a">银行财务配置，</span>
                    {/if}
                    {if condition="$vo.caiwu eq 1"}
                    <span style="color:#009688">财务数据统计，</span>
                    {/if}
                    {if condition="$vo.user eq 1"}
                    <span style="color:#00bcd4">会员管理中心，</span>
                    {/if}
                    {if condition="$vo.zuser eq 1"}
                    <span style="color:#ff9800">真人视讯体育，</span>
                    {/if}
                    {if condition="$vo.super eq 1"}
                    <span style="color:red">超级管理控制，</span>
                    {/if}
                    {if condition="$vo.huodong eq 1"}
                    <span style="color:#795548">平台活动管理，</span>
                    {/if}
                    {if condition="$vo.huodongnr eq 1"}
                    <span style="color:#2196f3">活动内容管理，</span>
                    {/if}
                    {if condition="$vo.yunwei eq 1"}
                    <span style="color:#ffc107">数据运维管理，</span>
                    {/if}
                    {if condition="$vo.qiyue eq 1"}
                    <span style="color:#e91e63">合同契约管理</span>
                    {/if}
