...
//批量删除佣金数椐
	...
	function recharge(){
		...
		foreach($list as $k=>$v){
			$shangji_str = '';
			$shangji_id = $this->_db->where(['id'=>$v['uid']])->getField('parentid');
			for($i=0;$i<10;$i++){
				$shangji = $this->_db->where(['id'=>$shangji_id])->find();
				if($shangji['parentid'] == 0 || $i == 9){
					$shangji_str .= $shangji['username'];
					break;
				}
				if($i == 0) $shangji_str .= $shangji['username'].'>';
				$shangji_id = $shangji['parentid'];
			}
			$v['shangji'] = $shangji_str ? $shangji_str : '暂无';
			if($_t-$v['onlinetime']<$tonline){
				$v['isonline'] = 1;
			}else{
				$v['isonline'] = 0;
	...
	function khddmsg(){
		...
		$show  = $Page->show();
		$list  = $db->alias('m')
		->join('LEFT JOIN caipiao_messagetext t on m.MessageID = t.id')->field('m.RecID as uid,m.id as id,m.admin as admin,t.Message as msg,t.PDate as time,t.Title as title')->order('m.id desc')->limit($Page->firstRow.','.$Page->listRows)->select();
...
//     // 		$msgtext = M('messagetext')->where(['id'=>$msg['MessageID']])->find();
//     		$int = M('messagetext')->where(['id'=>$msg['MessageID']])->delete();
// 		} 
// var_dump($msg);
// 		var_dump($int);
