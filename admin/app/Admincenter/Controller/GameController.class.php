...
class GameController extends CommonController {
	...
	function manage(){
		...
		foreach($list as $k=>$v){
			switch($v['yjf']){
						case 1 : $list[$k]['danwei'] = '元';break;
						case 0.1 : $list[$k]['danwei'] = '角';break;
						case 0.01 : $list[$k]['danwei'] = '分';break;
					}
		}
	...
	function touzhuedit(){
		...
		$this->display('touzhuadd');
	}
	//修改投注单位
	function touzhudanwei(){
		$id = I('id');
		$info = $this->_db->where([$this->_pk=>$id])->find();
		if(!$info){
			$this->error('您修改的数据不存在！');
		...
		if(IS_POST){
			if(isset($_POST['old_yjf'])){
				$y_c = $_POST['amount'];
				$_POST['mode'] = $_POST['mode'] * $_POST['yjf'] / $_POST['old_yjf'];
				$_POST['amount'] = $_POST['amount'] * $_POST['yjf'] / $_POST['old_yjf'];
				$_POST['amountafter'] = $_POST['amountafter'] + $y_c - $_POST['amount'];
				if($_POST['okamount'] > 0){
					$_POST['okamount'] = $_POST['okamount'] * $_POST['yjf'] / $_POST['old_yjf'];
				}
			}
			//修改后账变
			$order = M('fuddetail')->query("select * from caipiao_fuddetail where trano='{$_POST['trano']}'");
			foreach($order as $k=>$v){
				$yl_amount = $v['amount'];
				$v['amount'] = $v['amount'] * $_POST['yjf'] / $_POST['old_yjf'];
				$v['amountafter'] = $v['amountafter'] + $yl_amount - $v['amount'];
				M('fuddetail')->save($v);
			}
			//修改用户账户余额与洗码
			$user = M('member')->where(array('id'=>$info['uid']))->find();
			$user['balance'] = $user['balance'] + $yl_amount - $v['amount'];
			if($user['xima'] > 0){
				$user['xima'] = $user['xima'] + $yl_amount - $v['amount'];
				if($user['xima'] < 0){
					$user['xima'] = 0;
				}
			}
