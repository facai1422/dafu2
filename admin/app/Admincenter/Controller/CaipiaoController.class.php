...
/*		if($rate && !is_numeric($rate)){
	...
	function kai1(){
	    $kai = M('lhckai')->where(['id'=>1])->setField(['kai1st'=>1,'kai2st'=>0,'kai3st'=>0]);
	    $kai?$this->success('操作成功'):$this->error('操作失败');
	}
	function kai2(){
	    $kai = M('lhckai')->where(['id'=>1])->setField(['kai1st'=>0,'kai2st'=>1,'kai3st'=>0]);
	    $kai?$this->success('操作成功'):$this->error('操作失败');
	}
	function kai3(){
	    $kai = M('lhckai')->where(['id'=>1])->setField(['kai1st'=>0,'kai2st'=>0,'kai3st'=>1]);
	    $kai?$this->success('操作成功'):$this->error('操作失败');
	}
}