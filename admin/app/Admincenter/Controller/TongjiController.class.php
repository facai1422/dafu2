...
//		$where['isopen'] = 1;
		...
		$map2['oddtime'][]    = ['egt',strtotime(date('Y-m-d'))];
        $map2['oddtime'][]    = ['elt',time()];
        $tadaytouzhu =  M('touzhu')->Distinct(true)->field('uid')->where($map2)->where(['isnb'=>0])->order('id desc')->select();
        // var_dump(count($tadaytouzhu));die;  
        $this->assign('tadaytouzhu',count($tadaytouzhu));
