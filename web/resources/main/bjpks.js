...
var lotteryopencodes = function(lotteryname){
	...
	// 同时调用新的PK10开奖API更新显示
	loadLatestPCPK10DataFromNewAPI(lotteryname);
}
...
// 新增：PC端PK10调用新的开奖数据API
function loadLatestPCPK10DataFromNewAPI(lotteryType) {
	$.ajax({
		...
		success: function(res) {
			if (res.code === 200 && res.data && res.data.length > 0) {
				...
				updatePCPK10DisplayFromMainAPI(latest);
			}
		},
		error: function(xhr, status, error) {
			console.log('PC端PK10新API获取开奖数据失败:', error);
		}
	});
}

// 新增：PC端PK10更新页面显示（使用新API数据）
function updatePCPK10DisplayFromMainAPI(data) {
	if (!data) return;
	...
	if (data.opencode_array && data.opencode_array.length >= 10) {
		...
		for (var i = 0; i < Math.min(10, data.opencode_array.length); i++) {
			if ($sscList.eq(i).length > 0) {
				$sscList.eq(i).removeClass('bjpks_winning_sum_gif').text(data.opencode_array[i]);
				// 更新开奖球的样式类
		...
		data.opencode_array.forEach(function(num, index) {
			...
			numbersHtml += '<span style="display:inline-block; width:35px; height:35px; line-height:35px; text-align:center; background:linear-gradient(45deg, ' + color + ', rgba(255,255,255,0.3)); color:white; border-radius:50%; margin:3px; font-weight:bold; font-size:16px; box-shadow: 0 3px 6px rgba(0,0,0,0.3); border: 2px solid #fff;">' + num + '</span>';
		});
		$('#pc-latest-pk10-display .pc-pk10-lottery-numbers').html(numbersHtml);
		$('#pc-latest-pk10-display').show();
	}
}