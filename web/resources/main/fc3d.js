...
/*		var data = {
	  ...
	  // 同时调用新的福彩3D开奖API更新显示
	  loadLatestFC3DDataFromNewAPI(lotteryname);
}
...
// 新增：PC端福彩3D调用新的开奖数据API
function loadLatestFC3DDataFromNewAPI(lotteryType) {
	$.ajax({
		...
		success: function(res) {
			if (res.code === 200 && res.data && res.data.length > 0) {
				...
				updatePCFC3DDisplayFromMainAPI(latest);
			}
		},
		error: function(xhr, status, error) {
			console.log('PC端福彩3D新API获取开奖数据失败:', error);
		}
	});
}

// 新增：PC端福彩3D更新页面显示（使用新API数据）
function updatePCFC3DDisplayFromMainAPI(data) {
	if (!data) return;
	...
	if (data.opencode_array && data.opencode_array.length >= 3) {
		...
		$('#pc-latest-fc3d-display .pc-fc3d-lottery-numbers').html(numbersHtml);
		$('#pc-latest-fc3d-display').show();
	}
}