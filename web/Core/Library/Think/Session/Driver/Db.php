 ...
 * `session_data` varchar(255) NOT NULL COMMENT 'SESSION值',
 * `session_expire` int(10) unsigned NOT NULL DEFAULT '0' COMMENT 'SESSION更新时间',
...
class Db extends SessionHandler
{
    ...
    public function read($session_id)
    {
        ...
        if ($this->config['expire'] != 0) {
            $map['session_expire'] = ['gt', time() - $this->config['expire']];
        }
    ...
    public function write($session_id, $session_data)
    {
        ...
        $data   = ['session_id' => $this->config['prefix'] . $session_id, 'session_expire' => time(), 'session_data' => $session_data];

        if ($result) {

              $affect_rows = $this->handler->where('session_id', $this->config['prefix'] . $session_id)->update($data);
    ...
    {
        if ($this->config['expire'] != 0) {
            $map['session_expire'] = ['lt', time() - $this->config['expire']];
        } else {
            $map['session_expire'] = ['lt', time() - $sessMaxLifeTime];
        }
