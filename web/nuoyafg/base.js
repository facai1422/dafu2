...
LC.base = (function($) {
    ...
    var init = function(args) {
        opt = $.extend(opt, args);
        // start LC.base.init()
        $('#CustomerService_01, .contact_us').unbind();
        $('#CustomerService_01, .contact_us').click(function(){
        ...
        // var randomNum = Math.floor(Math.random() * 1000 + 8000 );
        // $(".online_number").html(randomNum);

        $(document).on('mouseenter', '.Personal_nav_box .game-lobby-nav, .Personal_nav_box .game-lobby-menu-wrapper', function(event) {
            $( ".Personal_nav_box .game-lobby-menu-wrapper" ).show();
        });

        $(document).on('mouseleave', '.Personal_nav_box, .Personal_nav_box .game-lobby-menu-wrapper', function(event) {
            $( ".Personal_nav_box .game-lobby-menu-wrapper" ).hide();
        });

        $(document).on('click', '.lightbox .lightbox-cancel, .lightbox .lightbox-close', function(event) {
            // 弹窗取消
    ...
    // 账户余额
    var dataRefreshBalance = function(args){
        opt = $.extend(opt, args);
        $('body').on('click', '[data-refresh-balance]', function() {
            ...
            $.get(opt.balanceUrl, function(resp) {
    ...
    return {
        init: init,
        dataRefreshBalance: dataRefreshBalance
    };
})(jQuery);
//弹框
function showNewWin(url, dwidth, dheigh) {
    ...
    window.open(url,"_blank","innerHeight=" + dheigh + ", width=" + dwidth + ", innerWidth=" + dwidth + ", height=" + dheigh + "top=" + iTop + ", left=" + iLeft + ",toolbar=no,menubar=no,scrollbars=yes,resizeable=yes,location=no,status=no");
};