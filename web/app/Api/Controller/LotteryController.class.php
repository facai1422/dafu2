...
class LotteryController extends CommonController {
	...
	function cpbuy($apiparam=array()){
		...
		$switchdlbuy = GetVar('switchdlbuy');
// 		 if($switchdlbuy!=1 & $userinfo['proxy']==1){
// 		 	$apiparam['sign']=false;
// 		 	$apiparam['message']='您是尊贵的代理,管理员已关闭代理投注';
...
// var_dump($lotteryname);
// var_dump($_lottetimes["openRemainTime"]);
// var_dump($_lottetimes["remainTime"]);
// die;    
        if($lotteryname == 'amlhc' || $lotteryname == 'lhc'){
            if($_lottetimes["openRemainTime"] >= $_lottetimes["remainTime"]){
                $apiparam['sign'] = false;
    			$apiparam['message'] = '当前彩种已截至投注0';
    			return $apiparam;
            }
        }
		if($_lottetimes['currFullExpect']!=$expect){            //根椐期号判断彩种是否过期
		...
		foreach($_REQUEST['orderList'] as $k=>$v){
			...
			if(intval($rates[$v['playid']]['maxxf'])>0){
				...
				// 	  if(strstr($_REQUEST['lotteryname'],'twdlt'))$zyclass ="lhc";
					  if(strstr($_REQUEST['lotteryname'],'k3'))$zyclass ="k3";
				// 	  var_dump($_REQUEST['lotteryname']);die; 
		...
		foreach($_REQUEST['orderList'] as $k=>$v){
			...
			//下注反拥  先判断开关
			$fanyong_type=GetVar('fanyong_type');
			
			if(in_array($typeid,$type_fanyong) && $userinfo['parentid'] !='0' && $agentCommissionOpen && $fanyong_type==1){
				$i = 1;
				$this->dailifandian($userinfo['parentid'],$userinfo['fandian'],$data['amount'],$trano,$userinfo['id'],$userinfo['username'],$userinfo['fandian'],$i);
             foreach($this->_parent as $k => $v){
				...
				if(round($userinfo['fandian'],2)>=round($v['fandian'],2)){
							break;
				}
					
					//返点金额必须大于最低限制
				if(round( $v['fandianjine'],2)<round(GetVar('fanyong_min'),2)){
					continue;	
				}
