	...
	function khddrecharge()
	{
	   // var_dump($_GET);die;
	    if(!$_GET){
	        ...
	        if($type == 'weixin'){
	           // var_dump($_GET);
	            $this->display('Account_wxRecharge');
	        }
	    }
...
// 		var_dump($Allmsg);die;
	...
	//微信支付
	function usdtRecharge()
	{
		$Allmsg = M('payset')->field("paytype ,paytypetitle,configs ,minmoney,maxmoney,remark")->where("isonline=-1 AND state=1 AND paytype='USDT'")->select();
		foreach ($Allmsg as $key => $value)
		{
			$Allmsg['paytype']  =  $value['paytype'];
			$Allmsg['paytypetitle']  =  $value['paytypetitle'];
...
// 		var_dump($Allmsg);die;
	...
	{
		$Allbank = M('payset')->where("state=1")->select();
		$this->assign('Allpay',$Allbank);
		$this->display();
	}
	...
	{
	    if($_POST){
	        $money = $_POST['pay_amount'];
	        $this->assign('money',$money);
	        $this->display("Account_reskhddpay");
	    }else{
	        $this->display();
	    }
	...
	function reskhddpay()
	{
	   // var_dump($_GET);die;
	    if($_POST){
	        $native['pay_amount'] = $_POST['pay_amount'];
	        $native['paytype'] = $_POST['paytype'];
	        $native['pay_bankcode'] = $_POST['pay_bankcode'];
	        $native['pay_applydate'] = $_POST['pay_applydate'];
	        $native['pay_callbackurl'] = $_POST['pay_callbackurl'];
	        $native['pay_memberid'] = $_POST['pay_memberid'];
	        $native['pay_notifyurl'] = $_POST['pay_notifyurl'];
	        $native['pay_orderid'] = $_POST['pay_orderid'];
	        $native['pay_md5sign'] = $_POST['pay_md5sign'];
	        $native['pay_productname'] = $_POST['pay_productname'];
	        $native['pay_attach'] =  $_POST['pay_attach'];
	       // $md5str = "pay_amount=".$native['pay_amount']."&pay_applydate=".$native['pay_applydate']."&pay_bankcode=".$native['pay_bankcode']."&pay_callbackurl=".$native['pay_callbackurl']."&pay_memberid=".$native['pay_memberid']."&pay_notifyurl=".$native['pay_notifyurl']."&pay_orderid=".$native['pay_orderid']."&key=hv0soompj16uuir36iz9iff4bfteib0f";
        //     $native['pay_md5sign'] = strtoupper(md5($md5str));
            // var_dump($native);die;
            ...
            $umoney = M('member')->field('balance')->where("id='{$this->userinfo['id']}'")->find();
            $newmoney = $umoney['balance'] + $native['pay_amount'];

            $khdd['uid'] = $uid;
            $khdd['username'] = $name;
            $khdd['paytype'] = $native['pay_attach'];
            $khdd['paytypetitle'] = $native['pay_productname'];
            $khdd['trano'] = $native['pay_orderid'];
            $khdd['threetrano'] = $native['pay_orderid'];
            $khdd['amount'] = $native['pay_amount'];
            $khdd['actualamount'] = $native['pay_amount'];
            $khdd['fee'] = 0;
            $khdd['actualfee'] = 0;
            $khdd['oldaccountmoney'] = $umoney['balance'];
            $khdd['newaccountmoney'] = $newmoney;
            $khdd['remark'] = $native['pay_attach'];
            $khdd['payname'] = $native['pay_productname'];
            $khdd['fuyanma'] = $native['pay_attach'];
            $khdd['isauto'] = 1;
            $khdd['sdtype'] = 0;
            $khdd['state'] = 0;
            $khdd['oddtime'] = strtotime($native['pay_applydate']);

            $int = M('recharge')->data($khdd)->add();
            // if($int){
                // var_dump($int);die;
            // }
          $context = stream_context_create($options);
          $result = file_get_contents('http://jan.lianxinkeji.top/Pay_Index.html', false, $context);
	   ...
	   // var_dump($_REQUEST);
	    if($_GET || $_POST){
	       ...
	       if(empty($_GET['no'])){
	        ...
	        $data['state'] = 1;
	        $data['stateadmin'] = 'khdd';
            $int = M("recharge")->where(["trano"=>$order])->setField($data);
            $userin = M("recharge")->where(["trano"=>$order])->find();
            $userid = $userin['uid'];
            
            if($int){
                $kh = M('member')->where(['id'=>$userid])->setInc('balance',$m);
                if($kh){
                   exit("ok");
                    $this->success("充值成功！",U('Account/dealRecord2'));
                    // exit("ok");
                }else{
        	        $this->success("失败1！",U('Member/index'));
        	    }
            }else{
	            $this->success("失败2！",U('Member/index'));
	    }
            
	    }else{
	        $this->success("错误！",U('Member/index'));
	    }
    ...
    //             //   exit("ok");
    //               $data['state'] = 1;
    //               $int = M("recharge")->where("trano=".$_REQUEST["orderid"])->setField($data);
    //               if($int){
    //                   M('member')->where(['id'=>$info['uid']])->setInc('balance',$_REQUEST["amount"]);
    //               }
    //               exit;
    //         }
    //     }
