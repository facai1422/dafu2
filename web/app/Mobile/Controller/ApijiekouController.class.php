...
// 		if(!$userinfo)exit('未登录');
        ...
        $data=M('message a')
            ...
            ->select();
            foreach ($data as $k => $v) {
            	if(strpos($v['read'],'"'.$RecID.'"'))
            		unset($data[$k]);

            }
            // var_dump($data);die;
            $json=['number'=>count($data)];
	...
	}/*CREATE TABLE `caipiao_contract_log` (
  `member_id` int(10) unsigned NOT NULL COMMENT '自增ID',
  `days` tinyint(2) unsigned DEFAULT NULL COMMENT '0->未签约 1->已签约',
  `mues` tinyint(2) unsigned DEFAULT NULL COMMENT '0->未签约 1->已签约',
  `yues` tinyint(2) unsigned DEFAULT NULL COMMENT '0->未签约 1->已签约',
  `add_time` int(10) unsigned NOT NULL COMMENT '添加时间',*/
	public function addcontractlog()
	{
		if(!IS_POST){
			echo jsonreturn(['sign'=>false,'message'=>'非法操作']);exit;
		}
		$typDb = array('days','mues' , 'yues');
		
		if( $this->userinfo['proxy'] == 1) {
			$typedays = I('post.type'); 
			$vlues   = I('post.vlues');
			if( !in_array($typedays, $typDb)) {
				echo jsonreturn(['sign'=>false,'message'=>'契约类型不存在']);exit;
			}
			$member_id =$this->userinfo['id'];
			$params = array();
			$res = M("ContractLog")->where('member_id='.$member_id)->find();
			$params = array($typedays=>$vlues);
			$params[$typedays.'_time'] = time();
			if(!empty($res)){
				$typHows = isset($res[$typedays]) ? $res[$typedays] : 0;
				if( $typHows == 1){
					echo jsonreturn(['sign'=>false,'message'=>'你已经成功签约过了']);exit;
				}
				//修改
				$bool = M("ContractLog")->where('member_id='.$member_id)->save($params);
			} else {
				$params['member_id'] = $member_id;
				$params['add_time'] = time();
				$bool = M("ContractLog")->add($params);
			}	
			echo jsonreturn(['sign'=>$bool ? true : false,'message'=>$bool ? '签约成功' : '签约失败']);exit;
		} else {
			echo jsonreturn(['sign'=>false,'message'=>'不是代理人员，不能进行签约']);exit;
		}
	}
	public function drawshare()
	{
		if(!IS_POST){
			echo jsonreturn(['sign'=>false,'message'=>'非法操作']);exit;
		}
		...
		$Cres = M("Contract")->where('id=1')->find();
		if( empty($Cres) ){
			echo jsonreturn(['sign'=>false,'message'=>'契约不存在']);exit;
		}
		$member_id =$this->userinfo['id'];
		$params = array();
		$res = M("ContractLog")->where('member_id='.$member_id)->find();
		...
		$money_formats= 0;
		$mues_transferIn = 0;
		if( $typedays == "days" && $res['days'] == 1){
			$is_type = 0;
			$cres_money = $Cres['days'];
			$startime =  !empty($res['days_time']) ?  date("Y-m-d",$res['days_time']) : '';
			$endtime =   !empty($res['days_time']) ?  date("Y-m-d",strtotime("+1 day",$res['days_time'])) : '';
			$resCount = $this->reportstatistics_drawshare($startime , $endtime);
			$mues_transferIn = isset($resCount['data']['transferIn']) ?  $resCount['data']['transferIn']: 0;
		} else  if( $typedays == "mues" && $res['mues'] == 1) {
			$is_type = 1;
			$cres_money = $Cres['mues'];
			$startime =  !empty($res['mues_time']) ?  date("Y-m-d",$res['mues_time']) : '';
			$endtime =   !empty($res['mues_time']) ?  date("Y-m-d",strtotime("+1 week",$res['mues_time'])) : '';
			$resCount = $this->reportstatistics_drawshare($startime , $endtime);
			$mues_transferIn = isset($resCount['data']['transferIn']) ?  $resCount['data']['transferIn']: 0;
		} else if( $typedays == "yues" && $res['yues'] == 1){
			$is_type = 2;
			$cres_money = $Cres['yues'];
			$startime =  !empty($res['yues_time']) ?  date("Y-m-d",$res['yues_time']) : '';
			$endtime =   !empty($res['yues_time']) ?  date("Y-m-d",strtotime("+1 month",$res['yues_time'])) : '';
			$resCount = $this->reportstatistics_drawshare($startime , $endtime);
			$mues_transferIn = isset($resCount['data']['transferIn']) ?  $resCount['data']['transferIn']: 0;
		}
		$params = array(
			'member_id' => $member_id,//用户ID
			'is_type'   => $is_type,//0->日契约  1->周契约 2->月契约
			'target'    => $cres_money,//充值目标金额
			'percent'   => $Cres['ratio'],//分红百分比
			'ramount'   => $mues_transferIn,//充值金额
			'start_time'=> strtotime($startime),//签约时间
			'end_time'  => strtotime($endtime),//结束签约时间
			'status'    => 0,//0->申请中  1->已经领取 2->已拒绝
			'drawshare_amount' => $cres_money*$Cres['ratio'],//分红金额
		);
		//查看是否有重复、
		$wmap['member_id'] = $member_id;
		$wmap['is_type']   = $is_type;
		$wmap['status'] = 0;
		$res = M("ContractDrawshare")->where($wmap)->find();
		if( !empty($res) ){
			echo jsonreturn(['sign'=>false,'message'=>'你已经领取了，正在审核 ']);exit;
		}
		$bool = M("ContractDrawshare")->add($params);
		if( $bool ){
			echo jsonreturn(['sign'=>true,'message'=>'你已经领取了，正在审核']);exit;
		} else {
			echo jsonreturn(['sign'=>false,'message'=>'你已经领取了，正在申请中']);exit;
			
		}
	}
	
	function reportstatistics_drawshare($startime , $endtime){
	...
	function gettouzhuinfo(){
		...
		switch ($Result['BillInfo']['yjf']){
        case 1 : $Result['BillInfo']['yjf_m'] = '元';break;
        case 0.1 : $Result['BillInfo']['yjf_m'] = '角';break;
        case 0.01 : $Result['BillInfo']['yjf_m'] = '分';break;
        case 0.001 : $Result['BillInfo']['yjf_m'] = '厘';break;
    }
		echo jsonreturn($Result);
	}
	function chedan(){
