...
class MemberController extends CommonController {
	...
	function xinnum(){
	    $xinyong = M('member')->where("id=".$this->userinfo['id'])->getField('xin_num');
	    $this->assign('xin' , $xinyong);
	    $this->display();
	}
	function setting(){ 
		$schedule = 30;
...
//		if($this->userinfo['modify'])$schedule += 15;
	...
	function qiyue()
	{
		...
		$clres = M("ContractLog")->where("member_id=".$member_id)->find();
		$startime = null;
		$endtime  = null;
		$days_transferIn = 0;
		$mues_transferIn = 0;
		...
		if( !empty($clres) ){
			//日
			if( $clres['days'] == 1){
				$startime =  !empty($clres['days_time']) ?  date("Y-m-d",$clres['days_time']) : '';
				$endtime =   !empty($clres['days_time']) ?  date("Y-m-d",strtotime("+1 day",$clres['days_time'])) : '';
				$xiaoTime = $endtime." 00:00:00";
				$daTime = $endtime." 23:59:59";
				$xiao_time  = strtotime($xiaoTime);
				$da_time  = strtotime($daTime);
				$resCount = $this->reportstatistics($startime , $endtime);
				$days_transferIn = isset($resCount['data']['transferIn']) ?  $resCount['data']['transferIn']: 0;
				if( $totime >= $xiao_time  && $totime <= $da_time){

					
					//在规定的时间里面没有够，则过期
					if($mues_transferIn < $res['days']){
						$dblwamp = array();
						$dblwamp['days'] = 0;
						$dblwamp['days_time'] = '';	
						M("ContractLog")->where("member_id=".$member_id)->save($dblwamp);
						$days_zufeData = false;
					}
				}
				$cdwherap = array("member_id"=>$member_id , "is_type"=> 0 , 'status'=> 2);
				$cdres = M("ContractDrawshare")->where($cdwherap)->find();
				if( !empty($cdres)){
					$dblwamp = array();
						$dblwamp['days'] = 0;
						$dblwamp['days_time'] = '';	
			...
			//周
			if( $clres['mues'] == 1){
				$startime =  !empty($clres['mues_time']) ?  date("Y-m-d",$clres['mues_time']) : '';
				$endtime =  !empty($clres['mues_time']) ?  date("Y-m-d",strtotime("+1 week",$clres['mues_time'])) : '';
				$xiaoTime = $endtime." 00:00:00";
				$daTime = $endtime." 23:59:59";
				$xiao_time  = strtotime($xiaoTime);
				$da_time  = strtotime($daTime);
				...
				$cdwherap = array("member_id"=>$member_id , "is_type"=> 1 , 'status'=> 2);
				$cdres = M("ContractDrawshare")->where($cdwherap)->find();
				if( !empty($cdres)){
			...
			//月
			if( $clres['yues'] == 1){
				$startime =  !empty($clres['yues_time']) ?  date("Y-m-d",$clres['yues_time']) : '';
				$endtime =  !empty($clres['yues_time']) ?  date("Y-m-d",strtotime("+1 month",$clres['yues_time'])) : '';
				$xiaoTime = $endtime." 00:00:00";
				$daTime = $endtime." 23:59:59";
				$xiao_time  = strtotime($xiaoTime);
				$da_time  = strtotime($daTime);
				$resCount = $this->reportstatistics($startime , $endtime);
				$yues_transferIn = isset($resCount['data']['transferIn']) ?  $resCount['data']['transferIn']: 0;
				if( $totime >= $xiao_time  && $totime <= $da_time){
					
					//在规定的时间里面没有够，则过期
					if($mues_transferIn < $res['yues']){
						$dblwamp = array();
						$dblwamp['days'] = 0;
						$dblwamp['days_time'] = '';	
						M("ContractLog")->where("member_id=".$member_id)->save($dblwamp);
				...
				$cdwherap = array("member_id"=>$member_id , "is_type"=> 2 , 'status'=> 2);
				$cdres = M("ContractDrawshare")->where($cdwherap)->find();
				if( !empty($cdres)){
					$dblwamp = array();
						$dblwamp['days'] = 0;
		...
		$days_transferIns = !empty($days_transferIn) ? $days_transferIn/($res['days']/100):0;
		$mues_transferIns = !empty($mues_transferIn) ? $mues_transferIn/($res['mues']/100): 0;
		$yues_transferIns = !empty($yues_transferIn) ? $yues_transferIn/($res['yues']/100): 0;
		$days_arr = array('transferIn'=>$days_transferIn , 'percent' => $days_transferIns > 100 ? 100 : $days_transferIns);
		$mues_arr = array('transferIn'=>$mues_transferIn , 'percent' => $mues_transferIns > 100 ? 100 : $mues_transferIns);
		$yues_arr = array('transferIn'=>$yues_transferIn , 'percent' => $yues_transferIns > 100 ? 100 : $yues_transferIns);

		$this->assign('mues_zufeData' , $mues_zufeData);
		$this->assign('days_zufeData' , $days_zufeData);
		$this->assign('yues_zufeData' , $yues_zufeData);
		$this->assign('proxy',$this->userinfo['proxy']);
		$this->assign('days_arr',$days_arr);
		$this->assign('days_arr',$days_arr);
		$this->assign('mues_arr',$mues_arr);
		$this->assign('yues_arr',$yues_arr);
		$this->assign('res',$res);
		$this->assign('clres',$clres);
		$this->display();
	}
	
	
	function reportstatistics($startime , $endtime){
