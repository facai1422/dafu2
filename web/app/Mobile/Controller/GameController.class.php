...
//k3
	function k3(){
		...
		$_allcp = M('Caipiao')->field('id,title,typeid,name,zhibao_url')->where("isopen='1' AND iswh='0'")->order('listorder ASC')->select();
	    $cpRes = array();
		foreach($_allcp as $key=>$val) {
			if($val['name'] == $lotteryname ){
				$cpRes = $_allcp[$key];
			}
		}
		$this->assign('cpRes',$cpRes);
		$typeid = 'k3';
